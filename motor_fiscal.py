...
def gerar_excel_final(df_ent, df_sai, file_ger_ent=None, file_ger_sai=None):
    ...
    # Auditoria ICMS
    df_audit_icms = df_sai.copy() if not df_sai.empty else pd.DataFrame()
    if not df_audit_icms.empty:
        ...
        def audit_r(row):
            ...
            st_e = "✅ ST Localizado" if ncm in ncm_st else "❌ Sem ST na Entrada"
            ...
            return pd.Series([st_e, "✅" if abs(row['ALQ-ICMS']-aliq_e)<0.1 else "Divergente", format_brl(row['VPROD']), format_brl(row['BC-ICMS']*aliq_e/100), "Ajustar"])
        df_audit_icms[['ST Entrada', 'Diagnóstico', 'VLR PROD', 'ICMS ESP', 'AÇÃO']] = df_audit_icms.apply(audit_r, axis=1)
    ...
    with pd.ExcelWriter(mem, engine='xlsxwriter') as wr:
        ...
        if not df_audit_icms.empty: df_audit_icms.to_excel(wr, sheet_name='AUDIT_ICMS', index=False)
