...
if st.session_state['confirmado']:
    if not st.session_state['garimpo_ok']:
        ...
        if uploaded_files and st.button("🚀 INICIAR GRANDE GARIMPO"):
            ...
            # --- STATUS VISUAL DO GARIMPO ---
            progresso_bar = st.progress(0)
            status_text = st.empty()
            total_arquivos = len(uploaded_files)
            
            with st.status("⛏️ Inicializando motores...", expanded=True) as status_box:
                with zipfile.ZipFile(buf_org, "w", zipfile.ZIP_STORED) as z_org, \
                    ...
                    for i, f in enumerate(uploaded_files):
                        ...
                        pct = (i + 1) / total_arquivos
                        progresso_bar.progress(pct)
                        status_text.text(f"⛏️ Processando arquivo {i+1} de {total_arquivos}: {f.name}")
