...
def identify_xml_info(content_bytes, client_cnpj, file_name):
    ...
    resumo = {"Arquivo": nome_puro, "Chave": "", "Tipo": "Outros", "Série": "0", "Número": 0, "Status": "NORMAIS", "Pasta": "", "Valor": 0.0, "Conteúdo": content_bytes, "Ano": "0000", "Mes": "00"}
    
    try:
        content_str = content_bytes[:45000].decode('utf-8', errors='ignore')
        tag_l = content_str.lower()
        if '<?xml' not in tag_l and '<inf' not in tag_l and '<inut' not in tag_l: return None, False
        
        # 1. PRIORIDADE: INUTILIZAÇÃO (Evita cair na trap de 'cancelamento' na justificativa)
        if '<inutnfe' in tag_l or '<retinutnfe' in tag_l or '<procinut' in tag_l:
            resumo["Status"], resumo["Tipo"] = "INUTILIZADOS", "NF-e"
            if '<mod>57</mod>' in tag_l: resumo["Tipo"] = "CT-e"
            elif '<mod>65</mod>' in tag_l: resumo["Tipo"] = "NFC-e"
            ...
            # 2. LOGICA PARA NOTAS NORMAIS E CANCELAMENTOS
            ...
            # Status: Busca apenas códigos específicos de evento, nunca o texto genérico da justificativa
            status = "NORMAIS"
...
with st.container():
    ...
    with m_c1:
        st.markdown("""<div class="instrucoes-card"><h3>📖 Manual de Integridade</h3><ul><li><b>Identificação Fiscal:</b> Extração direta da Chave ou das tags de Inutilização.</li><li><b>Critério Emitente:</b> Documentos cruzados com o CNPJ configurado.</li><li><b>Tripla Auditoria:</b> Primeiro valida Buracos, depois cruza Canceladas e Inutilizadas.</li></ul></div>""", unsafe_allow_html=True)
    with m_c2:
        st.markdown("""<div class="instrucoes-card"><h3>📊 Dashboard de Auditoria</h3><ul><li><b>Peneira em 3 Colunas:</b> Gaps reais vs Gaps cobertos por fiscal.</li><li><b>ZIP Estruturado:</b> Organização cronológica por Emitente e Status.</li></ul></div>""", unsafe_allow_html=True)
...
if st.session_state['confirmado']:
    if not st.session_state['garimpo_ok']:
        uploaded_files = st.file_uploader("Arraste os arquivos aqui:", accept_multiple_files=True)
        ...
        st.divider()
        st.download_button("📂 BAIXAR ORGANIZADO (ZIP)", st.session_state['z_org'], "garimpo_organizado.zip", use_container_width=True)
